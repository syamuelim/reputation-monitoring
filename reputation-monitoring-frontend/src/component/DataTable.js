      ...
      <TableContainer sx={{ maxHeight: apx(330), height: apx(330) }}>
        <Table stickyHeader aria-label="sticky table">
              ...
              {columns.map((column) => (
                <TableCell
                  ...
                  style={{
                    ...
                    backgroundColor: theme.palette.coreOrange.main,
                    color: "#fff",
                  }}
                >
                  <Box
          ...
          <ThemeProvider theme={theme}>
            <TableBody>
              {state.influencers.map((row, index) => {
                return (
                  <TableRow
                    hover
                    role="checkbox"
                    tabIndex={-1}
                    key={row.id}
                    sx={{ backgroundColor: index % 2 === 1 ? "#dff4ed" : "" }}
                  >
                    {columns.map((column) => {
                      const value = column.object
                        ? row[column.object][column.id]
                        : row[column.id];
                      return (
                        <TableCell key={column.key} align={column.align}>
                          {column.format && typeof value === "number" ? (
                            ...
                            <Chip
                              avatar={
                                <Avatar
                                  src={row.youtubeChannel.iconUrl}
                                ></Avatar>
                              }
                              label={value}
                              variant={"filled"}
