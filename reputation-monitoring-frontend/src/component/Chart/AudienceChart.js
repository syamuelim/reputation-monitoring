...
function AudienceChart() {
  ...
  let loadAudience = async () => {
    ...
    state.influencers.forEach(async (influencer) => {
      if (influencer.selected == true) {
        ...
        const audiecneResponse = await instagramService.getInstagramPostPyId(
            influencer.instagramUser.id,
            0, 100
        );
  ...
  function formDataObject(data) {
    let kol = {};
    if (data.length > 0) {
      kol = state.influencers.find((obj) => obj.instagramId == data[0].instagramId);
    }
    var dataObject = {
      data: data.map(({ likes }) => likes),
